...
const TreeAndShrubDepartment = () => {
  const data = departmentData.find((item) => item.name === "Lawn maintenance department");
  return (
    <div className="container p-0 my-3">
      <div className="row m-0 p-0 ">
        ...
        <div className="col-lg-9 col-md-9 col-12 bg-white p-3 list_content">
          <h3 className="list-category-heading">{data.name}</h3>
                ...
                data.submenu.map((item, index) => (
                  <Link to={item.name} key={index}>
                    <div className="tree-shrub-2main">
                      <div>
                        <img src={item.image} alt="img" className="tree-shrub-img" />
                      </div>
