...
function PorterandMatronDepartment() {
  const carpentryData = departmentData.find(
    (item) => item.name === "Porter and matron department"
  );
            ...
            <div className="d-flex flex-wrap gap-1 justify-content-center align-items-center">
              {maindata.map((item, index) => (
                <Link to={item.name} key={index}>
                  <div className="text-white absolute" id="octa-div">
                    {item.name}
                  </div>
                </Link>
              ))}
...
export default PorterandMatronDepartment;
