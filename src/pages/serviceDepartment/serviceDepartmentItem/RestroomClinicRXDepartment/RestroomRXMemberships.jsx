                    ...
                    {item.button&&<div className="d-flex justify-content-center">
                      <div>
                        <label className="rx-switch-button">
                          <input type="checkbox" />
                          <span className="rx-slider-btn text-center " onClick={()=>clickHandler(item.type,index)}>{item.type}</span>
                        </label>
                      </div>
                    </div>}
                    <div className="RxMembershipCostDiv">
                        <p> ${item.type==='year'?item?.cost2*12:item?.cost2} </p>
                      {/* <Link to={item.name} target="_blank" className="d-flex align-items-end"><button> More... </button></Link> */}
                    </div>
                    {(index < 3) &&
                      <Payment
                        memberShipPlan={Number(item?.cost2)}
